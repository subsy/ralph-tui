...
<Callout type="warning">
  Check your task source first - the cause differs depending on whether you're
  using prd.json or Beads.
</Callout>
...
<Callout type="tip">
  If you installed an agent with a different package manager (npm, cargo, pip),
  you may need to add its bin directory to your PATH, or use the `command`
  config option to specify the full path.
</Callout>
...
<Callout type="warning">
  Force overriding or deleting the lock file can lead to parallel runs. Only do
  this if you're certain no other session is running.
</Callout>
...
<Callout type="info">
  Rate limiting is common with high-volume usage. Ralph TUI has built-in
  strategies to handle this gracefully.
</Callout>
