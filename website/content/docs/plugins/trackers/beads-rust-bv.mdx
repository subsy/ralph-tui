...
<Step>Create and initialise a Beads project with `br`:
...
<Step>Install the `bv` (beads-viewer) CLI for smart task selection:
...
<Step>Verify both tools are on your `PATH`:
...
<Steps>
<Step>Create an epic and child tasks using `br`:
...
<Step>Link dependencies so `bv` can analyse them:
...
<Step>Run Ralph TUI with the `beads-rust-bv` tracker:
