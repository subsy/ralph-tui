...
<Callout type="tip">
  The JSON tracker is ideal for single-developer projects or when you want to
  try Ralph TUI without installing additional tools.
</Callout>
...
<Step title="Run Ralph TUI">```bash ralph-tui run --prd ./prd.json ```</Step>
...
<Callout type="warning">
  The `--prd` flag is recommended over config-file path because you likely have
  different prd.json files for different features.
</Callout>
