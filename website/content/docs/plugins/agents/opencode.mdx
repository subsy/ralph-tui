...
<Callout type="tip">
  OpenCode lets you use any supported AI provider with Ralph TUI, including
...
<Callout type="tip">
  For simple configs, you can use the top-level `command` option instead of the
  `[[agents]]` array: ```toml agent = "opencode" command =
  "/custom/path/opencode" ``` See [Custom
  Command](/docs/configuration/options#custom-command) for details.
</Callout>
...
<Callout type="info">
  Model names are validated by the provider's API. Invalid model names result in
  errors from OpenCode, not Ralph TUI.
</Callout>
...
<Callout type="info">
  Variant values are model-specific and validated by the provider's API. Check
  your model's documentation for supported variants.
</Callout>
...
<Callout type="warning">
  Using non-default agent types may trigger warning messages in OpenCode output.
  Ralph TUI filters these automatically, but you may see them if running
  OpenCode directly.
</Callout>
...
<Callout type="warning">
  Unlike Claude Code, OpenCode does not support subagent tracing. The tracing
  panel will be empty when using OpenCode.
</Callout>
...
<Step title="Pull a Model">
  ```bash ollama pull llama3 # or ollama pull codellama ```
</Step>
...
<Callout type="tip">
  Local models avoid API rate limits and costs, but may be slower and less
  capable for complex coding tasks.
</Callout>
