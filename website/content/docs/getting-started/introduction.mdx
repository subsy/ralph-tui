...
<Callout type="tip">
  Ralph TUI connects your AI coding assistant (Claude Code, OpenCode, Factory
  Droid) to your task tracker (prd.json, Beads) and runs them in an autonomous
  loop, completing tasks one-by-one with intelligent selection, error handling,
  and full visibility.
</Callout>
...
<Callout type="info">
  Manual AI-assisted coding works well for single tasks, but becomes tedious
  when working through feature backlogs. Ralph TUI eliminates the
  copy-paste-wait cycle.
</Callout>
...
<Callout type="tip">
  After running `ralph-tui setup`, skills are installed to `~/.claude/skills/`
  and can be invoked directly in Claude Code. This gives you the power of
  Ralph's PRD and task creation without leaving your preferred agent interface.
</Callout>
