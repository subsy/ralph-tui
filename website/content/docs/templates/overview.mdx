...
## Instructions 1. Implement this user story following the acceptance criteria
2. Run your project's quality checks (typecheck, lint, etc.) 3. Commit your
changes with a descriptive message When finished, signal completion with:
...
{{#if epicId}}
  - **Epic**:
  {{epicId}}{{#if epicTitle}} - {{epicTitle}}{{/if}}
{{/if}}
...
## Instructions 1. Implement the requirements (stay on current branch) 2. Run
your project's quality checks 3. Commit: feat:
{{taskId}}
-
{{taskTitle}}
4. Close the bead when done When finished, signal completion with:
<promise>COMPLETE</promise>
```
...
<Step title="User Config Directory">
  Templates in `~/.config/ralph-tui/`: - `prompt.md` for JSON tracker -
