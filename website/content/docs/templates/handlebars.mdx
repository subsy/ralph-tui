...
```handlebars
\{\{#if epicTitle\}\} **Epic**: \{\{epicTitle\}\} \{\{else\}\} *No epic
assigned* \{\{/if\}\}
```
...
```handlebars
\{\{#unless dependsOn\}\} This task has no dependencies - you can start
immediately! \{\{/unless\}\}
```
...
```handlebars
\{\{!-- This is a Handlebars comment --\}\} \{\{! Short comment syntax \}\}
```
...
```handlebars
\{\{#if acceptanceCriteria\}\} ## Acceptance Criteria \{\{acceptanceCriteria\}\}
\{\{/if\}\}
```
...
```handlebars
\{\{#if dependsOn\}\} **Blocked by**: \{\{dependsOn\}\} Complete these first
before starting. \{\{/if\}\} \{\{#if blocks\}\} **Impact**: Completing this
unblocks: \{\{blocks\}\} \{\{/if\}\}
```
...
```handlebars
\{\{#if recentProgress\}\} ## Previous Progress \{\{recentProgress\}\}
\{\{/if\}\}
```
...
```handlebars
\{\{#if prdContent\}\} ## PRD: \{\{prdName\}\} \{\{#if prdDescription\}\}
\{\{prdDescription\}\} \{\{/if\}\} ### Progress:
...
```handlebars
\{\{#if codebasePatterns\}\} ## Codebase Patterns (Study These First)
\{\{codebasePatterns\}\} \{\{/if\}\}
```
...
```handlebars
\{\{#if selectionReason\}\} ## Why This Task Was Selected
\{\{selectionReason\}\} \{\{/if\}\}
```
...
```handlebars
\{\{#if priority\}\} **Priority**: P\{\{priority\}\} \{\{/if\}\}
```
...
```handlebars
\{\{!-- Access task metadata --\}\} \{\{#if task.metadata.branch\}\} Branch:
\{\{task.metadata.branch\}\} \{\{/if\}\} \{\{!-- Access nested properties --\}\}
\{\{#with task\}\} ID: \{\{id\}\} Title: \{\{title\}\} \{\{#each labels\}\} -
\{\{this\}\} \{\{/each\}\} \{\{/with\}\}
```

<Callout type="warning">
  Raw object access exposes internal structure that may change. Use documented
  variables when possible for forward compatibility.
</Callout>
...
```handlebars
\{\{#if epicId\}\} \{\{#if epicTitle\}\} **Epic**: \{\{epicId\}\} -
\{\{epicTitle\}\} \{\{else\}\} **Epic**: \{\{epicId\}\} \{\{/if\}\} \{\{/if\}\}
```
...
```handlebars
\{\{!-- These render the same in Ralph --\}\} \{\{taskDescription\}\} \{\{{taskDescription\}\}}
```
...
```handlebars
\{\{task.metadata.customField\}\} \{\{config.tracker.plugin\}\}
```
...
```handlebars
\{\{#if type\}\} **Type**: \{\{type\}\} \{\{else\}\} **Type**: task \{\{/if\}\}
```
...
```handlebars
\{\{#if epicId\}\} ## Epic Context - **Epic**: \{\{epicId\}\} - **Title**:
\{\{epicTitle\}\} \{\{/if\}\}
```
...
```handlebars
\{\{!-- Just the date --\}\} Started: \{\{currentDate\}\} \{\{!-- Full timestamp
...
```handlebars
\{\{!-- Wrong: Missing closing --\}\} \{\{#if condition} Content \{\{/if\}\}
\{\{!-- Wrong: Extra spaces in helper --\}\} \{\{ #if condition \}\} \{\{!--
Correct --\}\} \{\{#if condition\}\} Content \{\{/if\}\}
```
...
```handlebars
## Task: \{\{taskTitle\}\} \{\{taskDescription\}\} ## MANDATORY Quality Gates
Run ALL of these before signaling completion: 1. **Type Check**: `bun run
typecheck` - Must have ZERO errors 2. **Lint**: `bun run lint` - Must have ZERO
warnings 3. **Tests**: `bun run test` - All tests must pass DO NOT signal
completion if any check fails.
...
```handlebars
\{\{taskTitle\}\} \{\{acceptanceCriteria\}\} Done?
<promise>COMPLETE</promise>
```
...
```handlebars
\{\{#if prdContent\}\} ## PRD: \{\{prdName\}\} \{\{prdDescription\}\} ###
Progress: \{\{prdCompletedCount\}\}/\{\{prdTotalCount\}\} complete
<details>
  <summary>Full PRD</summary>
  \{\{prdContent\}\}
</details>
\{\{/if\}\} \{\{#if codebasePatterns\}\} ## Codebase Patterns
\{\{codebasePatterns\}\} \{\{/if\}\} # \{\{taskTitle\}\} ## Metadata | Field |
Value | |-------|-------| | ID | \{\{taskId\}\} | | Type | \{\{type\}\} | |
Priority | P\{\{priority\}\} | | Status | \{\{status\}\} | | Labels |
\{\{labels\}\} | | Tracker | \{\{trackerName\}\} | | Agent | \{\{agentName\}\} |
| Date | \{\{currentDate\}\} | \{\{#if epicId\}\} ## Epic: \{\{epicId\}\}
\{\{epicTitle\}\} \{\{/if\}\} ## Description \{\{taskDescription\}\} ##
Acceptance Criteria \{\{acceptanceCriteria\}\} \{\{#if dependsOn\}\} ##
Dependencies Depends on: \{\{dependsOn\}\} \{\{/if\}\} \{\{#if blocks\}\} ##
Impact Unblocks: \{\{blocks\}\} \{\{/if\}\} \{\{#if recentProgress\}\} ##
Previous Progress \{\{recentProgress\}\} \{\{/if\}\} \{\{#if selectionReason\}\}
## Selection Reason \{\{selectionReason\}\} \{\{/if\}\} ## Completion
<promise>COMPLETE</promise>
```
