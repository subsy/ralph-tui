...
{{#if blocks}}
  **Impact**: Completing this unblocks:
  {{blocks}}
{{/if}}
```
...
## MANDATORY Quality Gates Run ALL of these before signaling completion: 1.
**Type Check**: `bun run typecheck` - Must have ZERO errors 2. **Lint**: `bun
run lint` - Must have ZERO warnings 3. **Tests**: `bun run test` - All tests
must pass DO NOT signal completion if any check fails.
