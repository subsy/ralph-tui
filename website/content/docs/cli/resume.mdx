...
<Callout type="tip">
  Session state is automatically saved to `.ralph-tui/session.json`. You can
  resume even after crashes or forced exits.
</Callout>
...
<Callout type="warning">
  Only use `--force` when you're certain no other Ralph instance is running.
  Using it while another instance is active can cause conflicts.
</Callout>
...
<Step title="Interrupt">
  Press `q` to quit, or the process gets interrupted. Session state is saved
  automatically.
</Step>
