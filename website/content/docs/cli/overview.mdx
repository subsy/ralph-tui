...
- **[run --listen](/docs/cli/run#remote-listener)** - Enable WebSocket server for remote control (server side)
- **[run --remote-only](/docs/cli/run#remote-only-mode)** - Launch the TUI as a pure remote client — no local engine, no local tab
...
# Or launch the TUI as a pure client (no local engine, no local tab)
