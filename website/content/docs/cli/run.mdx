...
<Callout type="tip">
  Running `ralph-tui` without any command also starts the TUI interface,
  allowing you to navigate and start execution interactively.
</Callout>
...
<Callout type="info">
  Model names within each provider are validated by the provider's API. If you
  specify an invalid model name, you'll see an error from the underlying agent
  CLI.
</Callout>
