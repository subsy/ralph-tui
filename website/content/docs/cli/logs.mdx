...
<Callout type="tip">
  Log files are stored in `.ralph-tui/iterations/` by default. Use the
  `--output-dir` flag on `run` to change this location.
</Callout>
...
<Callout type="warning">
  The `--clean` operation permanently deletes log files. Use `--dry-run` first
...
<Step title="View Full Output">
  ```bash ralph-tui logs --iteration 4 --verbose ``` Review the complete agent
  output for error messages.
</Step>

<Step title="Check the Prompt">
  The log includes the prompt that was sent. Verify it contains the expected
...
<Callout type="info">
  Consider adding `.ralph-tui/iterations/` to your `.gitignore` to avoid
  committing large log files.
</Callout>
