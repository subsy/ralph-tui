...
<Callout type="info">
  Configuration is validated using Zod schemas at runtime. Invalid
  configurations will show helpful error messages pointing to the specific
  problem.
</Callout>
...
<Callout type="tip">
  For most users, the shorthand `agent` and `tracker` fields are sufficient. The
  array syntax is only needed for advanced multi-agent setups.
</Callout>
