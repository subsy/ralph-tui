...
| `model` | string | - | Default model override for the selected agent. The TUI Agent & Model picker saves here when **Save as default** is enabled. |
...
| `model` | string | - | Model passed to the selected agent. If omitted, plugin-specific defaults apply. |
