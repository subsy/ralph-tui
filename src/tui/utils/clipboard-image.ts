...
function getInstallHints(): Record<string, string> {
  ...
  switch (os) {
    ...
    case 'linux':
      return {
        xclip: 'Install xclip: sudo apt install xclip (Debian/Ubuntu) or sudo pacman -S xclip (Arch)',
        'wl-paste': 'Install wl-clipboard: sudo apt install wl-clipboard (Debian/Ubuntu) or sudo pacman -S wl-clipboard (Arch)',
 ...
 */
export async function hasClipboardImage(timeoutMs: number = DEFAULT_TIMEOUT_MS): Promise<boolean> {
  const os = platform();
  ...
  switch (os) {
    ...
    case 'linux': {
      ...
      if (await commandExists('wl-paste')) {
        const result = await execCommand('wl-paste', ['--list-types'], timeoutMs);
 ...
 */
async function readMacOSWithPngpaste(timeoutMs: number): Promise<ClipboardImageResult> {
 ...
 */
async function readMacOSWithOsascript(timeoutMs: number): Promise<ClipboardImageResult> {
  ...
  try {
    ...
    if (output.includes("Can't get the clipboard") || output.includes('-1700')) {
      return { data: null };
    }
 ...
 */
async function readLinuxWithXclip(timeoutMs: number): Promise<ClipboardImageResult> {
  const result = await execCommand(
    'xclip',
 ...
 */
async function readLinuxWithWlPaste(timeoutMs: number): Promise<ClipboardImageResult> {
  const result = await execCommand('wl-paste', ['--type', 'image/png'], timeoutMs);
 ...
 */
async function readWindowsClipboard(timeoutMs: number): Promise<ClipboardImageResult> {
