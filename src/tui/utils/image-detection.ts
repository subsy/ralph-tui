 ...
 */
const DATA_URI_PATTERN = /^data:image\/(png|jpe?g|gif|webp);base64,([A-Za-z0-9+/=]+)$/i;
 ...
 */
const BASE64_MAGIC_BYTES: ReadonlyArray<{ prefix: string; extension: ImageExtension }> = [
...
export function detectDataUriImage(input: string): Base64ImageResult {
  ...
  if (result.isBase64Image && !result.extension) {
    ...
    result.extension = (declaredType === 'jpg' ? 'jpeg' : declaredType) as ImageExtension;
  }
...
export function detectRawBase64Image(input: string): Base64ImageResult {
  ...
  const hasImageMagic = BASE64_MAGIC_BYTES.some((magic) => trimmed.startsWith(magic.prefix));
  if (!hasImageMagic) {
 ...
 */
function detectImageFormatFromBuffer(buffer: Buffer): ImageExtension | undefined {
  if (buffer.length < 12) {
    return undefined;
  }
