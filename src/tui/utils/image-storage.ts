...
export async function storeImageFromPath(
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to store image from path',
    };
  }
}
 ...
 * ```
 */
export async function listStoredImages(baseDir?: string): Promise<StoredImageInfo[]> {
  const storageDir = getStorageDir(baseDir);
...
export async function deleteStoredImage(
  ...
  try {
    ...
    const isFullPath = pathOrFilename.includes('/') || pathOrFilename.includes('\\');
