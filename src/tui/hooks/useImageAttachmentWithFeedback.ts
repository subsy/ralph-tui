 ...
 */
export interface UseImageAttachmentWithFeedbackReturn extends Omit<UseImageAttachmentReturn, 'attachImage' | 'attachFromClipboard'> {
...
function categorizeError(error: string, inputLooksLikePath: boolean): ErrorCategory {
  ...
  if (lowerError.includes('clipboard') || lowerError.includes('pngpaste') || 
      lowerError.includes('xclip') || lowerError.includes('wl-paste')) {
  ...
  // Storage failures
  if (lowerError.includes('store') || lowerError.includes('save') || lowerError.includes('write')) {
...
export function useImageAttachmentWithFeedback(
   ...
   * Attach an image from clipboard with toast feedback.
   */
  const attachFromClipboard = useCallback(async (): Promise<AttachResultWithFeedback> => {
   ...
   */
  const attachImage = useCallback(async (input: string): Promise<AttachResultWithFeedback> => {
    ...
    if (result.success) {
      ...
      const category = categorizeError(result.error ?? '', inputLooksLikePath);
