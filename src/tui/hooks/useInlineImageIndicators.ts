...
export function useInlineImageIndicators(): UseInlineImageIndicatorsReturn {
  ...
  const getIndicatorNumber = useCallback(
    (imageId: string): number => {
      if (indicatorMap[imageId] !== undefined) {
        return indicatorMap[imageId];
      }
  ...
  const handleBackspace = useCallback(
    (
      ...
      if (indicator) {
        ...
        const newText = text.slice(0, indicator.start) + text.slice(indicator.end);
